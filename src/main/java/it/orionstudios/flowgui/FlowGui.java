...
import it.orionstudios.flowgui.internal.SlotBatch;
import it.orionstudios.flowgui.item.GuiItem;
import it.orionstudios.flowgui.pagination.PageTarget;
...
public abstract class FlowGui {
    ...
    //Slot with PAGETARGET (PAGE(ALL))
    protected void slot(int slot, PageTarget target, GuiItem item) {

        if (target instanceof PageTarget.Single single) {
            slot(slot, single.page(), item);
            return;
        }

        if (target instanceof PageTarget.All) {
    ...
    public void applySlot(int slot, GuiItem item) {
        applySlot(slot, page, item);
    }

    protected GuiItem pageIndicator() {
        return GuiItem.of(org.bukkit.Material.PAPER)
                .name("§ePage " + (page() + 1) + " §7/ §e" + pages())
                .onClick(ctx -> {}); // non cliccabile
    }

    protected PageTarget page(int page) {
        return new PageTarget.Single(page);
    }

    protected PageTarget page(PageTarget.All all) {
        return all;
    }

    protected static final PageTarget.All all = PageTarget.All.INSTANCE;

    @SafeVarargs
    public static Consumer<GuiContext> actions(Consumer<GuiContext>... actions) {
        return ctx -> {
            for (Consumer<GuiContext> action : actions) {
                action.accept(ctx);
            }
        };
    }
    ...
    protected SlotBatch slots(int... slots) {
        return new SlotBatch (slots);
    }
